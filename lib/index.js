...
const getPathsToProfiles = (shell) => {
	const processEnv = processWrapper.getEnv();

	// In some environments HOME variable may not be set, but the ~ will work as it is resolved by `getent passwd`.
	const home = processEnv.HOME || '~';

	const currentProfileOrder = profileOrder.map(p => p.replace("bash", shell)) // in case the default shell is not bash
...
const getEnvironmentVariables = () => {
	...
	const shell = processEnv && processEnv.SHELL && path.basename(processEnv.SHELL) || "bash",
		pathToShell = processEnv && processEnv.SHELL || "/bin/bash",
	...
	let environmentVariables = _.cloneDeep(processEnv);
